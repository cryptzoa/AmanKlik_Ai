...
export function ScanClient(
  ...
  const previewUrl = useMemo(() => (file ? URL.createObjectURL(file) : null), [
    file,
  ]);
  ...
  useGSAP(() => {
    ...
    const tl = gsap.timeline({ paused: !isReady });
    timelineRef.current = tl;

    tl.from("[data-scan-panel]", {
      autoAlpha: 0,
      x: 24,
      duration: 0.38,
      ease: "power2.out",
    });
    tl.from("[data-fixture-chip]", {
      autoAlpha: 0,
      y: 10,
      stagger: 0.035,
      duration: 0.28,
    ...
    try {
      ...
      setError(
        demoError instanceof Error
          ? demoError.message
          : "Fixture screenshot belum tersedia.",
      );
    } finally {
  ...
  return (
    <div ref={root} className="mt-12">
      <div>
        ...
        >
          {tabs.map((tab) => (
            <button
              ...
              className={`relative min-h-12 border-b-2 px-5 text-sm font-semibold transition ${
                mode === tab.id
                  ? "border-ink text-ink"
        ...
        <div
          data-scan-panel
          className="mt-8 border border-ink bg-surface p-5 shadow-[8px_8px_0_rgba(17,17,17,0.075)] sm:p-8"
        >
          {mode === "text"
            ? (
              <label className="block" htmlFor="scan-text">
                ...
                />
                <span className="mt-2 block text-right font-mono text-xs text-muted">
            ...
            ? (
              <label className="block" htmlFor="scan-url">
                ...
                <span className="mt-3 block text-sm leading-6 text-muted">
                  AmanKlik tidak membuka situs ini; kami hanya memeriksa
                  struktur alamat dan konteks yang kamu kirim.
                </span>
              </label>
            )
            : (
          ...
          <div className="mt-8 flex flex-col gap-4 border-t border-line pt-5 sm:flex-row sm:items-center sm:justify-between">
            <p className="max-w-xl text-xs leading-5 text-muted">
              Versi demo menggunakan API AI pihak ketiga. Hindari mengunggah
              percakapan nyata yang berisi data pribadi atau informasi sensitif.
            </p>
            <MotionButton
              type="button"
              disabled={disabled || !canSubmit}
              className="editorial-button editorial-button-primary relative z-10 px-7 disabled:cursor-not-allowed"
              onClick={submit}
            >
        ...
        {status === "loading"
          ? (
            <div
              className="mt-5 border border-line bg-ai-soft p-4 text-sm"
              role="status"
            >
              <span className="mr-3 inline-block size-2 animate-pulse rounded-full bg-ai" />Memvalidasi
              input dan menyusun penjelasan…
            </div>
          )
          : null}
        {error
          ? (
            <p
              className="mt-5 border border-risk/30 bg-risk-soft px-4 py-3 text-sm text-ink"
              role="alert"
            >
              {error}
            </p>
            ...
            {mode === "text"
              ? DEMO_TEXT_FIXTURES.map((fixture) => (
                <button
                  ...
                  key={fixture.id}
                  type="button"
                  className="lift-link rounded-full border border-line bg-surface px-4 py-2 text-sm font-semibold hover:border-ai hover:text-ai"
              ...
              : DEMO_IMAGE_FIXTURES.map((fixture) => (
                <button
                  ...
                  className="lift-link rounded-full border border-line bg-surface px-4 py-2 text-sm font-semibold hover:border-ai hover:text-ai disabled:opacity-50"
                  onClick={() => selectImageDemo(fixture.path, fixture.title)}
                >
                  {fixture.id} · {fixture.title}
                </button>
              ))}
      ...
      <aside className="mt-10 grid gap-8 border-t border-line pt-6 sm:grid-cols-[0.32fr_0.68fr]">
        <p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-ai">
          Alur pemeriksaan
        </p>
        <ol className="divide-y divide-line border-y border-line">
          {[
            [
              "01",
              "Input divalidasi",
              "Ukuran, format, dan isi diperiksa sebelum analisis.",
            ],
            [
              "02",
              "Sinyal dipisahkan",
              "Rules, URL intelligence, dan konteks AI dibaca terpisah.",
            ],
            [
              "03",
              "Aksi dijelaskan",
              "Hasil berisi alasan dan langkah aman, bukan hanya label.",
            ],
