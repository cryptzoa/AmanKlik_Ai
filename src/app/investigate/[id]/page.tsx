...
export default async function InvestigationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <InteriorShell eyebrow="07 / Case" title={investigation.title} description={investigation.summary} marker="ARTEFAK UNIK / POLA BERSAMA" fragments={[investigation.riskLevel.replace("_", " "), `${investigation.scans.length} ARTEFAK`, `${investigation.graph.nodes.filter((node) => node.kind === "signal" || node.kind === "domain").length} POLA`]} compact>
