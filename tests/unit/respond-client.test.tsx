...
describe("RespondClient", () => {
  it("supports multiple selections and resets the complete triage", () => {
    ...
    expect(
      screen.getByRole("heading", {
        name: /Hubungi bank atau e-wallet sekarang/i,
      }),
    ).toBeVisible();
    expect(screen.getByText(/Rencana diperbarui/i)).toHaveTextContent(
      "6 langkah perlu dilakukan sekarang",
    );
  ...
  it("uses the affected service to replace a generic recovery action", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: /Akun atau nomor HP sudah diambil alih/i,
      }),
    );
    expect(
      screen.getByRole("heading", {
        name: /Amankan akun yang paling penting/i,
      }),
    ).toBeVisible();

    fireEvent.click(screen.getByRole("button", { name: /Nomor HP atau SIM/i }));
    expect(screen.getByRole("heading", { name: /Hubungi operator seluler/i }))
      .toBeVisible();
    expect(
      screen.queryByRole("heading", {
        name: /Amankan akun yang paling penting/i,
  ...
  it("only shows relevant services and clears a stale selection", () => {
    ...
    fireEvent.click(transfer);
    expect(screen.getByRole("button", { name: /Bank, kartu, atau e-wallet/i }))
      .toBeVisible();
    expect(screen.getByRole("button", { name: /Marketplace/i })).toBeVisible();
    expect(screen.queryByRole("button", { name: /Email utama/i })).not
      .toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /WhatsApp/i })).not
      .toBeInTheDocument();
    ...
    fireEvent.click(takeover);
    expect(screen.getByRole("button", { name: /Email utama/i }))
  ...
  it("keeps unsure exclusive from concrete incidents", () => {
    ...
    const unsure = screen.getByRole("button", {
      name: /Saya tidak yakin apa yang sudah terjadi/i,
    });
    const transfer = screen.getByRole("button", {
      name: /Uang sudah terkirim/i,
    });
